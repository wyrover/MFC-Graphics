...
#define THRESHOLDMAX	1000000000.0//判断奇点的门限
#define THRESHOLDMIN	0.00001		//判断相同值的门限
...
void CExpressionDlg::OnPaint()
{
	...
	else
	{
		if(!pts.points.empty())
		{
			...
			if((pts.yend - pts.ybegin)>THRESHOLDMIN)
			{
				...
				while(itor != pts.points.end())
				{
					...
					if(temp == pts.ybegin || temp == pts.yend || xielv < -THRESHOLDMAX || xielv > THRESHOLDMAX)
					{
						dc.MoveTo(x,y);
					}
					else
			...
			{
				dc.MoveTo(0,rt.bottom/2);
				dc.LineTo(rt.right,rt.bottom/2);
			}
		}
	}
}
